prune.mosse <- function (phy) 
{
    ...
    if (sum(!to.drop) < 2) {
        ...
        phy2 <- drop.tip.mosse(phy, to.drop)
    	...
    	phy$keep.idx <- match(phy$edge[, 2],phy$orig$idx2)
  		phy$keep.tip <- match(1:length(phy$tip.label),phy$orig$idx2)
        phy
    }
}

drop.tip.mosse <- function(phy, to.drop, trim.internal = TRUE, subtree =
                           FALSE, root.edge = 0, rooted = T) {
  tip <- phy$tip.label[to.drop]
  ...
  #phy$keep <- keep
  phy$keep.idx <- match(edge2[keep],phy$orig$idx2)
  phy$keep.tip <- match(which(!to.drop),phy$orig$idx2)
